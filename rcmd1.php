...
$email = $_POST["email"];
//all the users
$random_q = "select name, graph from Game where name not in (select name from likes where email = '{$eamil}') order by rand() limit 6";
$result = mysqli_query($conn, $random_q);
$random = array();
while ($row = mysqli_fetch_assoc($result)) $random[] = $row;


//$rcmd = $users[array_search(max($common), $common)];
...
<div class='demo-charts mdl-color--white mdl-shadow--2dp mdl-cell mdl-cell--5-col mdl-grid'>
...
<tbody>
    ...
    ");
for ($i = 0; $i < 3; $i++) {
    print("<tr><td>{$random[$i]['name']}</td></tr>");
    }
for ($i = 0; $i < 3; $i++) {
    print("<tr><td><img src={$random[$i]['graph']} height='100' width='70'/></td></tr>");
    }
for ($i = 0; $i < 3; $i++) {
    print("<tr>
				<td width='70'>
				<form action='add_friend.php' method='POST' enctype='multipart/form-data'>
						...
						<input type='text' name='email2' value='{$random[$i]['name']}' style='display: none'>
