<?php
  ...
  $games = $_POST['games'];
	...
	$query = "SELECT MAX(CID) FROM Comments";
	$res = mysqli_query($conn, $query);
	$row = mysqli_fetch_array($res, MYSQLI_NUM);
	//print($row[0]);

	$column_list = "(CID, games, score, email, content)";
	$value_list = "({$row[0]} + 1, '{$games}', {$score}, '{$email}', '{$content}')";
	$query = "INSERT INTO Comments ".$column_list." VALUES ".$value_list;
	$res = mysqli_query($conn, $query);
